-- BotiRestaurant — init schema
-- Перенесено з boti-restaurant/sql/001-init-restaurant-schema.sql у Supabase
-- migrations format, щоб GitHub ↔ Supabase інтеграція авто-застосовувала.
-- Ідемпотентний — якщо вручну вже виконував в SQL Editor, повтор не зашкодить.
...
-- ────────────────────────────────────────────────────────────────────────
-- RLS — базовий бутстреп: всім відкриваємо read та (поки) write.
-- Наступна міграція (20260526120100_secure_rls.sql) замінить anon-write на
-- authenticated-write. Ця потрібна зараз, щоб вперше засіяти дані
-- без логіну.
-- ────────────────────────────────────────────────────────────────────────
