-- =====================================================================
-- Paddy's Point — заміна демо-меню на реальне меню з PDF (159 позицій)
-- Запускається на проді (Supabase Preview integration), бо seed.sql
-- не виконується на персистентній БД.
-- =====================================================================
...
declare
  v_tenant_id uuid;
begin
  select id into v_tenant_id from public.tenants where slug = 'paddys';

  if v_tenant_id is null then
    raise notice 'Tenant "paddys" not found — skipping menu replacement';
    return;
  end if;

  -- Очищення: menu_items видаляємо першими (categories має on delete restrict).
  -- menu_item_translations і daily_specials каскадно тягнуться за menu_items.
  delete from public.menu_items where tenant_id = v_tenant_id;
  delete from public.categories where tenant_id = v_tenant_id;
end $$;

-- Нові категорії за структурою офіційного PDF-меню Paddy's Point La Zenia.
...
items (sort_order, cat_slug, name, description, price, is_vegetarian, tags) as (
  values
    ...
    -- 8. SANDWICHES / PANINIS / BAGUETTES / TOASTIES (до 18:00)
